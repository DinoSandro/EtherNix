# git-diff
# Autogenerated from man page /run/current-system/sw/share/man/man1/git-diff.1.gz
...
complete -c git-diff -l diff-algorithm -d 'Choose a diff algorithm'
...
complete -c git-diff -o 'X[<param1' -l dirstat -d 'Output the distribution of relative amount of changes for each sub-directory'
complete -c git-diff -l '*stat' -d options
complete -c git-diff -l color-moved -d 'Moved lines of code are colored differently.  It can be changed by the diff'
complete -c git-diff -l color-moved-ws -d 'This configures how whitespace is ignored when performing the move detection …'
complete -c git-diff -l word-diff -d 'Show a word diff, using the <mode> to delimit changed words'
...
complete -c git-diff -s S -d 'Binary files are searched as well'
...
complete -c git-diff -o O/dev/null -d 'The output order is determined by the order of glob patterns in <orderfile>'
...
complete -c git-diff -l cc -d 'option is used): diff --cc file  2. c   2.  4'
complete -c git-diff -s O -d 'option to git-diff(1) for details.  If diff'
complete -c git-diff -s l -d 'diff. renames Whether and how Git detects renames'
