# git-diff-index
# Autogenerated from man page /run/current-system/sw/share/man/man1/git-diff-index.1.gz
...
complete -c git-diff-index -l diff-algorithm -d 'Choose a diff algorithm'
...
complete -c git-diff-index -o 'X[<param1' -l dirstat -d 'Output the distribution of relative amount of changes for each sub-directory'
complete -c git-diff-index -l '*stat' -d options
complete -c git-diff-index -l color-moved -d 'Moved lines of code are colored differently'
complete -c git-diff-index -l color-moved-ws -d 'This configures how whitespace is ignored when performing the move detection …'
complete -c git-diff-index -l word-diff -d 'Show a word diff, using the <mode> to delimit changed words'
...
complete -c git-diff-index -s S -d 'Binary files are searched as well'
...
complete -c git-diff-index -o O/dev/null -d 'The output order is determined by the order of glob patterns in <orderfile>'
...
complete -c git-diff-index -l cc -d 'option is used): diff --cc file  2. c   2.  4'
