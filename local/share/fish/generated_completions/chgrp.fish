...
complete -c chgrp -l from -d 'change the ownership of each file only if its current owner and/or group matc…'
...
complete -c chgrp -l reference -d 'use RFILE\'s ownership rather than specifying values RFILE is always dereferen…'
...
complete -c chgrp -s P -d 'do not traverse any symbolic links'
