# git-remote
# Autogenerated from man page /run/current-system/sw/share/man/man1/git-remote.1.gz
...
complete -c git-remote -l tags -d 'option, git fetch <name> imports every tag from the remote repository.  With'
...
complete -c git-remote -l delete -d 'refs/remotes/<name>/HEAD is deleted.  With'
...
complete -c git-remote -l add -d 'get-url Retrieves the URLs for a remote'
complete -c git-remote -l push -d With
complete -c git-remote -l all -d 'set-url Changes URLs for the remote'
