# git-config
# Autogenerated from man page /run/current-system/sw/share/man/man1/git-config.1.gz
...
complete -c git-config -o trailing-space -d 'oc o 2'
...
complete -c git-config -l no-keep-cr -d 'from the command line.  See git-am(1), git-mailsplit(1).  am'
...
complete -c git-config -l whitespace -d 'option.  See git-apply(1).  blame'
complete -c git-config -l date -d 'option at git-log(1).  blame'
complete -c git-config -l ignore-revs-file -d blame
...
complete -c git-config -l origin -d 'command-line option to git-clone(1).  clone'
complete -c git-config -l reject-shallow -d 'option on the command line.  See git-clone(1) clone'
complete -c git-config -l filter -d 'in git-rev-list(1)) and'
complete -c git-config -l recurse-submodules -d 'is used, also apply the filter to submodules.  color'
complete -c git-config -l color -d 'option.  color. diff. <slot> Use customized color for diff colorization'
...
complete -c git-config -s C -d 'filename filename prefix (when not using'
complete -c git-config -s h -d 'function function name lines (when using'
complete -c git-config -s n -d 'column column number prefix (when using'
complete -c git-config -l column -d 'match matching text (same as setting matchContext and matchSelected) matchCon…'
complete -c git-config -l grep
complete -c git-config -l author
complete -c git-config -l committer -d 'selected non-matching text in selected lines'
...
complete -c git-config -l '*stat' -d options
complete -c git-config -l inter-hunk-context -d 'command line option.  diff'
complete -c git-config -s O -d 'option to git-diff(1) for details.  If diff'
complete -c git-config -l ws-error-highlight -d 'overrides this setting.  diff'
complete -c git-config -l color-moved-ws -d 'in git-diff(1).  diff'
complete -c git-config -l tool -d 'option in git-difftool(1) for more details.  difftool. <tool>'
complete -c git-config -l trust-exit-code -d 'option in git-difftool(1) for more details.  difftool'
...
complete -c git-config -l negotiation-tip -d 'options to git-fetch(1).  fetch. showForcedUpdates Set to false to enable'
complete -c git-config -l no-show-forced-updates -d 'in git-fetch(1) and git-pull(1) commands.  Defaults to true.  fetch'
...
complete -c git-config -l force-in-body-from -d 'option to format-patch.  Defaults to false.  format'
complete -c git-config -l cover-from-description -d 'option in git-format-patch(1).  format'
...
complete -c git-config -l filename-max-length -d 'command line option.  format'
...
complete -c git-config -l stdout -d 'is in use to escape "^>+From " lines.  format'
complete -c git-config -o p0 -d 'option.  filter. <driver>'
complete -c git-config -l depth -d 'option when'
complete -c git-config -l aggressive -d 'isn\'t in use.  See the documentation for the'
complete -c git-config -l window -d 'of 10.  See the documentation for the'
...
complete -c git-config -l full-name -d 'option by default.  grep'
complete -c git-config -l gpg-sign -d 'See gitformat-signature(5) for the signature format, which differs based on t…'
complete -c git-config -l negotiate -d 'curl(1)) oc o 2. 3 ntlm'
complete -c git-config -l ntlm -d 'curl(1)) http'
...
complete -c git-config -l no-abbrev-commit -d 'log. date Set the default date-time mode for the log command'
complete -c git-config -l decorate -d 'option of the git log.  log'
complete -c git-config -l decorate-refs-exclude -d 'command-line option, but the config option can be overridden by the'
complete -c git-config -l decorate-refs -d 'option.  log. diffMerges Set diff format to be used when'
...
complete -c git-config -l show-signature -d log
complete -c git-config -l use-mailmap
complete -c git-config -l no-use-mailmap -d 'lsrefs. unborn May be "advertise" (the default), "allow", or "ignore"'
...
complete -c git-config -l ff-only -d 'option from the command line).  merge'
complete -c git-config -l no-autostash -d and
complete -c git-config -l autostash -d 'options of git-merge(1).  Defaults to false.  merge'
...
complete -c git-config -l strategy -d 'option to git-notes(1).  notes. <name>'
...
complete -c git-config -l no-follow-tags -d 'push. gpgSign May be set to a boolean value, or the string if-asked'
...
complete -c git-config -l no-force-if-includes -d 'at the time of push overrides this configuration setting.  push'
complete -c git-config -l autosquash -d 'option of git-rebase(1) by default for interactive mode'
complete -c git-config -l no-autosquash -d 'option.  rebase'
complete -c git-config -l update-refs -d 'option by default.  rebase'
complete -c git-config -l exec -d 'option was provided).  This is the same as specifying the'
complete -c git-config -l reschedule-failed-exec -d 'option.  rebase. forkPoint If set to false set'
complete -c git-config -l no-fork-point -d 'option by default.  rebase. rebaseMerges Whether and how to set the'
...
complete -c git-config -l mirror -d 'option was given on the command line.  remote. <name>'
complete -c git-config -l refetch -d 'option of git-fetch(1).  remotes'
complete -c git-config -l pack-kept-objects -d 'was passed.  See git-repack(1) for details'
complete -c git-config -l write-bitmap-index -d 'or repack. writeBitmaps).  repack'
complete -c git-config -l delta-islands -d 'was passed.  Defaults to false.  repack'
complete -c git-config -l reference -d 'option is given.  safe'
complete -c git-config -l git-dir -d 'command-line option, or the GIT_DIR environment variable (see git(1))'
complete -c git-config -l shared -d 'option in git-init(1)).  This is a multi-valued setting, i. e'
...
complete -c git-config -l batch-size -d 'option of git-send-email(1).  sendemail'
complete -c git-config -l relogin-delay -d 'option of git-send-email(1).  sendemail'
complete -c git-config -s G -d '(print configuration) option and will subsequently use OpenSSH options (if th…'
complete -c git-config -s o -d 'oc o 2. 3 simple'
complete -c git-config -s P -d 'oc o 2. 3 tortoiseplink'
