# git-format-patch
# Autogenerated from man page /run/current-system/sw/share/man/man1/git-format-patch.1.gz
...
complete -c git-format-patch -l diff-algorithm -d 'Choose a diff algorithm'
...
complete -c git-format-patch -o 'X[<param1' -l dirstat -d 'Output the distribution of relative amount of changes for each sub-directory'
complete -c git-format-patch -l '*stat' -d options
...
complete -c git-format-patch -o O/dev/null -d 'The output order is determined by the order of glob patterns in <orderfile>'
