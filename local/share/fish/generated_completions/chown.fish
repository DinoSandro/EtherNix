...
complete -c chown -l from -d 'change the ownership of each file only if its current owner and/or group matc…'
...
complete -c chown -l reference -d 'use RFILE\'s ownership rather than specifying values RFILE is always dereferen…'
...
complete -c chown -s P -d 'do not traverse any symbolic links'
