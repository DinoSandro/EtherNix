...
complete -c cp -s n -l no-clobber -d '(deprecated) silently skip existing files.  See also --update'
...
complete -c cp -l update -d 'control which existing files are updated; UPDATE={all,none,none-fail,older(de…'
complete -c cp -s u -d 'equivalent to --update[=\\,older\\/].   See below'
complete -c cp -s v -l verbose -d 'explain what is being done'
complete -c cp -l keep-directory-symlink -d 'follow existing symlinks to directories'
